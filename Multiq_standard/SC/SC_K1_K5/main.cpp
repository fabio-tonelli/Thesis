...
int main(int argc, char** argv) {
  int i1_start = 0, i1_end = 15;
  std::string outfile = "results.csv";

  if (argc >= 3) {
    i1_start = std::atoi(argv[1]);
    i1_end = std::atoi(argv[2]);
  }
  if (argc >= 4) {
    outfile = argv[3];
  }
  ...
  fprintf(out, "K1,K5,C11,C12,C44,L100,T100,L110,T110_1,T110_2,L111,T111\n");
  ...
  for (int i1 = i1_start; i1 < i1_end; i1++) {
    ...
    for (int i3 = 0; i3 < N; i3++) {
      ...
      char cmd[256];
      snprintf(cmd, sizeof(cmd), "variable K1 equal %g", k1);
      lammps_command(lmp, cmd);
      snprintf(cmd, sizeof(cmd), "variable K5 equal %g", k5);
      lammps_command(lmp, cmd);
      ...
      if (lammps_has_error(lmp)) {
        ...
        fprintf(stderr, "K1=%g K5=%g -> ERRORE LAMMPS: %s\n", k1, k5, errmsg);
      } else {
      ...
      if (!c11 || !c12 || !c44) {
        fprintf(stderr, "K1=%g K5=%g -> mancante: C11=%s C12=%s C44=%s\n", k1,
                k5, c11 ? "ok" : "NULL", c12 ? "ok" : "NULL",
                c44 ? "ok" : "NULL");
      } else {
