"""
Mappa 2D (K1 vs K5) che classifica ogni punto della griglia in base al
comportamento di cmp_T_L aggregato su TUTTE le direzioni q campionate
per quel (K1, K5), lette da results_Multiq.csv (lo sweep generico
...
Categorie, mutuamente esclusive:
  ...
  3) "misto"     -> tutto il resto (compresi 'm', 'y' non esclusivo, '=',
                    ...
                    non esistono punti (K1,K5) con 'y' ma senza 'm'
...
# --- Classificazione per (K1, K5), 3 categorie esclusive -----------------
def classify(group):
...
pivot = result.pivot(index='K5', columns='K1', values='code')
