...
int main(int argc, char** argv) {
  ...
  fprintf(out, "K2,K4,C11,C12,C44,L100,T100,L110,T110_1,T110_2,L111,T111\n");
  ...
  for (int i2 = i2_start; i2 < i2_end; i2++) {
    ...
    for (int i3 = 0; i3 < N; i3++) {
      ...
      snprintf(cmd, sizeof(cmd), "variable K4 equal %g", k4);
      lammps_command(lmp, cmd);
      ...
      if (lammps_has_error(lmp)) {
        ...
        fprintf(stderr, "K2=%g k4=%g -> ERRORE LAMMPS: %s\n", k2, k4, errmsg);
      } else {
        double* c11 = (double*)lammps_extract_variable(lmp, "C11all", nullptr);
        // ... resto come prima ...
      }
      ...
      if (!c11 || !c12 || !c44) {
        fprintf(stderr, "K2=%g K4=%g -> mancante: C11=%s C12=%s C44=%s\n", k2,
                k4, c11 ? "ok" : "NULL", c12 ? "ok" : "NULL",
                c44 ? "ok" : "NULL");
      } else {
