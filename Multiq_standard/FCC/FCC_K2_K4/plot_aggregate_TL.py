"""
Mappa 2D (K2 vs K4) che classifica ogni punto della griglia in base al
comportamento di cmp_T_L aggregato su TUTTE le direzioni q campionate
per quel (K2, K4), lette da results_Multiq.csv (lo sweep generico
...
Categorie, mutuamente esclusive:
  ...
  3) "misto"     -> tutto il resto (compresi 'm', 'y' non esclusivo, '=',
                    ...
                    non esistono punti (K2,K4) con 'y' ma senza 'm'
...
# --- Classificazione per (K2, K4), 3 categorie esclusive -----------------
def classify(group):
...
pivot = result.pivot(index='K4', columns='K2', values='code')
