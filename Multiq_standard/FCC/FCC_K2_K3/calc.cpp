...
int main() {
  ...
  fprintf(out,
          "K2,K3,C11,C12,C44,q_x,q_y,q_z,Rho*v1^2,Rho*v2^2,Rho*v3^2,L_coeff1,L_"
          "coeff2,L_coeff3,cmp_T_L\n");

  // per ogni coppia di valori k2-k3 devo costruire, per ogni terna qx,qy,qz ,
  ...
  // numero di righe (i.e. valori diversi della coppia k2-k3)
  ...
  // ============================================================
  {
    ...
    fprintf(
        out_special,
        "K2,K3,C11,C12,C44,direzione,q_x,q_y,q_z,e1,e2,e3,L_coeff1,L_coeff2,"
        "L_coeff3,cmp_T_L\n");
    ...
    for (double phi = 0; phi <= M_PI / 2; phi += delta_phi) {
      ...
      for (int i = 0; i < N_dati; i++) {
        ...
        if (!isfinite(C11) || !isfinite(C12) || !isfinite(C44) ||
          ...
          fprintf(stderr,
                  "K2=%g K3=%g -> mancante: C11=%s C12=%s C44=%s e1=%s e2=%s "
                  "e3=%s L_Coeff1=%s L_Coeff2=%s L_Coeff3=%s cmp_T_L=%c  \n",
                  k2, k3, C11 ? "ok" : "NULL", C12 ? "ok" : "NULL",
